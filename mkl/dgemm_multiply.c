//
// Created by Myungjun Son on 2017. 7. 20..
//
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        N = atoi(argv[1]);  //atoi는 뭘까 문자열을 정수로 변환
    }

    a = (double *) malloc(sizeof(double) * N * N);  //행렬을 만들기 위한 메모리 동적 할당
    ...
    a = (double *) malloc(sizeof(double) * N * N);  //flush 라는 건 입력 버프를 지워주는 역할이다
    b = (double *) malloc(sizeof(double) * N * N);
    c = (double *) malloc(sizeof(double) * N * N);
    init_arr(N, a);
    init_arr(N, b);
...
void Dgemm_multiply(double *a, double *b, double *c, int N) {
    ...
    double alpha = 1.0, beta = 0.;  //alpha beta는 스칼라이다
    int incx = 1;
    int incy = N;
